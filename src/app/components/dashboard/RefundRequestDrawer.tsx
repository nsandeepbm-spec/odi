...
import { Banknote, CheckCircle2, Loader2, X, XCircle } from 'lucide-react';
import { inrFromPaise } from './shared';
import type { RefundRow, RefundStatus } from '../../lib/api';
...
export function razorpayRefundLabel(row: RefundRow): { label: string; tone: 'ok' | 'fail' | 'wait' } {
  if (row.providerRefundId) {
    return { label: `Razorpay refunded · ${row.providerRefundId}`, tone: 'ok' };
  }
  if (row.providerError) {
    return { label: `Razorpay failed · ${row.providerError}`, tone: 'fail' };
  }
  if (row.status === 'completed' && !row.providerPaymentId) {
    return { label: 'Completed without Razorpay (COD / no online payment)', tone: 'ok' };
  }
  if (row.providerPaymentId) {
    return { label: 'Razorpay not refunded yet', tone: 'wait' };
  }
  return { label: 'No Razorpay payment on this order', tone: 'wait' };
}
...
type Props = {
  ...
  onClose: () => void;
  onReview: (row: RefundRow, decision: 'approved' | 'rejected', adminNote: string) => Promise<void>;
};

/** Right drawer — refund request details. Not the order page. */
export function RefundRequestDrawer({ open, row, busyAction, onClose, onReview }: Props) {
  ...
  const canAct = row.status === 'pending' || row.status === 'approved';
      ...
      >
        <div className="flex items-start justify-between gap-3 px-5 py-5 border-b border-white/[0.06] bg-[#0d0d0d] shrink-0">
          <div className="min-w-0">
            ...
            <h2 id="refund-request-title" className="text-lg font-black text-white truncate">
              {row.orderNumber}
            </h2>
            <div className="mt-2">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-lg border border-white/10 text-neutral-400 hover:text-white hover:bg-white/5 shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-5 py-6 space-y-6">
          <section>
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4">
              ...
              <Detail label="Refunded at" value={row.refundedAt ? formatDate(row.refundedAt) : null} />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4">
              <Detail label="Payment id" value={row.providerPaymentId} />
              <Detail label="Refund id" value={row.providerRefundId} />
              <Detail
                label="Status"
                value={
                  <span
