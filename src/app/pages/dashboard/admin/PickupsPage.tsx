    ...
    <div>
      ...
      <div className="relative z-10 mb-4 flex flex-wrap gap-2">
        <button
          ...
          className={`inline-flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-xl border transition-colors ${
            tab === 'needs'
        ...
        >
          ...
          Needs schedule
          <span className="text-[10px] font-black px-1.5 py-0.5 rounded-md bg-white/10">
            {needsRows.length}
          </span>
        </button>
        <button
      ...
      {tab === 'needs' && (
        <Card className="relative z-10 mb-4">
          <div className="px-5 py-4 flex flex-col lg:flex-row lg:items-end gap-4 bg-[#0d0d0d]">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                disabled={busy}
                className="px-3 py-2 rounded-xl bg-[#050505] border border-white/[0.08] text-sm text-white outline-none focus:border-cyan-500 [color-scheme:dark] disabled:opacity-50"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-neutral-500">
              ...
              <select
                ...
                disabled={busy}
                className="px-3 py-2 rounded-xl bg-[#050505] border border-white/[0.08] text-sm text-white outline-none focus:border-cyan-500 disabled:opacity-50"
              >
                {PICKUP_TIMES.map((t) => (
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-bold tracking-wide bg-gradient-to-r from-emerald-400 to-teal-500 text-white rounded-xl disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {bulkRunning ? (
            ...
            {bulkProgress && (
              <p className="text-xs font-medium text-neutral-400 self-center">
                {bulkProgress.done} / {bulkProgress.total}…
              </p>
            )}
          </div>
          {lastMessage && (
            <p className="px-5 pb-3 text-xs font-medium text-emerald-400">{lastMessage}</p>
          )}
        </Card>
      )}

      {tab === 'scheduled' && lastMessage && (
        <p className="relative z-10 mb-3 text-xs font-medium text-emerald-400 px-1">{lastMessage}</p>
      )}

      <Card className="relative z-10">
        <div className="px-6 py-4 border-b border-white/[0.04] flex flex-col sm:flex-row sm:items-center gap-3 bg-[#0d0d0d]">
          ...
          <p className="text-xs text-neutral-500">
            ...
            {tab === 'needs' ? ` · bulk gap ${PICKUP_GAP_MS}ms` : ''}
          </p>
        </div>
